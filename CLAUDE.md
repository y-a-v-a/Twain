...
- **The appex is hand-assembled — SPM cannot build `.appex` targets.** `TwainQuickLook` is a plain SPM executable whose entry point is swapped to Foundation's `_NSExtensionMain` at link time (`Package.swift` linker flags); `build.sh` bundles it into `Contents/PlugIns` with `quicklook/Info.plist` and signs it with `quicklook/TwainQuickLook.entitlements` *before* re-signing the outer app. The appex must stay sandboxed and must carry its own copies of the Textual and SwiftUIMath resource bundles (it is its own main bundle — without Textual's, code blocks print uncolored; without SwiftUIMath's, the first formula traps in `Bundle.module`). Math is enabled everywhere through `twainSyntaxExtensions` (Sources/TwainRendering/MarkdownSyntax.swift); every new parser must use it. `Tests/quicklook/verify-bundle.sh` (run in CI) checks all of this.
