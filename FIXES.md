...
- [ ] **Problem:** matches are computed against `HighlightingMarkdownCache.plainText`
...
- [ ] **Fix (minimal):** in `HighlightingMarkdownParser.attributedString`, before
...
- [ ] **Fix (preferred):** highlight the same `AttributedString` the cache already
...
- [ ] **Problem:** `updateQuery` / `nextMatch` / `previousMatch` bump
...
- [ ] **Fix:** reuse the cached base `AttributedString` on the render side and only
...
- [ ] **Problem:** `displayText` appends `\u{E000}` + `renderRevision` to force a
...
- [ ] **Fix:** drive the re-parse off an explicit revision input instead of
...
- [ ] **Problem:** during the refresh-while-searching window, the guard skips
...
- [ ] **Fix:** add a comment acknowledging the transient, or recompute the label
  from the matches actually rendered.
- Location: `SearchBar.swift:41-45,641-647`

### 5. Make `query` non-publicly-settable
- [ ] **Problem:** `var query` is freely settable; everything else uses
  `private(set)` + a mutator to keep `matches` in sync. A stray external write
  desyncs matches from the displayed query.
- [ ] **Fix:** make it `private(set) var query` and route all writes through
  `updateQuery`.
- Location: `SearchBar.swift:131,200-205`

### 6. Collapse to a single parse (single source of truth)
- [ ] **Problem:** two parser instances, two parses of the same input. Source of
  tasks 1 and 2.
- [ ] **Fix:** parse once into the cache; have the render-side `MarkupParser`
...
- [ ] **Problem:** harmless today (style code only reads size-independent fields),
  but reads like a bug and invites one.
- [ ] **Fix:** pass the live font size, or add a parameterless accessor for the
  size-independent metrics.
- Location: `ThemedStyle.swift:10,44,84,94,140,152`

### 8. Note the width-independent wrap estimate
- [ ] **Problem:** `estimatedLineUnits` assumes 72 chars/line regardless of window
  width, so scroll accuracy drifts as the window resizes.
- [ ] **Fix:** comment that this is a deliberate approximation (or feed actual
...
- [ ] **Problem:** reads global event state to choose direction — fragile coupling.
- [ ] **Fix:** rely on the existing Cmd-G / Cmd-Shift-G commands, or a dedicated
...
- [ ] `findMatches`: empty query; no match; overlapping candidates
  (`"aa"` in `"aaaa"` → 2 non-overlapping); case-insensitivity; multi-grapheme
  query (emoji/combining marks) → grapheme-based offsets.
- [ ] Offset-alignment invariant: across a markdown corpus,
  `cache.plainText.count` == render-side parsed `characters.count` (guards task 1).
- [ ] `rebuildMatches(resetSelection: false)`: current match preserved by
  `lowerBound` after an edit that shifts/removes earlier matches; index clamps
  when match count shrinks.
- [ ] `nextMatch` / `previousMatch`: wraparound at both ends; no-op when empty.
- [ ] Refresh-while-searching: `updateDocument` with an active query leaves
